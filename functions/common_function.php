                    ...
                    while($row=mysqli_fetch_assoc($result_query)){
                        ...
                        <div class='card'>
                            ...
                            <div class='card-body'>
                                ...
                                <p class='card-text'>Price: KSH: $product_price/=</p>
        ...
        if(!isset($_GET['brand'])){
                 $select_query="SELECT * FROM products order by rand()";
                    ...
                    while($row=mysqli_fetch_assoc($result_query)){
                        ...
                        $product_title=$row['product_title'];
                        $product_description=$row['product_description'];
//                        $product_keywprds=$row['product_keywords'];
                        $product_image1=$row['product_image1'];
                        $product_price=$row['product_price'];
                        $category_id=$row['category_id'];
                        $brand_id=$row['brand_id'];
...
function getbrands(){
    ...
    $select_brands="SELECT * FROM brands";
                     ...
                     while($row_data=mysqli_fetch_assoc($result_brands)){
                         ...
                         <a href='index.php?brands=$brand_id' class='nav-link text-light'>$brand_title</a></li>";
                     }
...
function cart(){
    if(isset($_GET['add_to_cart'])){
        global $con;
        $get_ip_add = getIPAddress();   //calling ip address
        ...
        if($num_of_rows>0){
            ...
            echo "<script>alert('Item is Added to Cart Successfully')</script>";
             echo "<script>window.open('index.php','_self')</script>"; //when u click ok
        }
    }
}
...
function cart_item(){
//    checks if cart  is active count number of rows
     if(isset($_GET['add_to_cart'])){
        global $con;
        $get_ip_add = getIPAddress();   //calling ip address 
...
function total_cart_price(){
    global $con;
    $get_ip_add = getIPAddress(); //accessing ip address of specific user
    $total_price=0;
    $cart_query="SELECT * FROM cart_details WHERE ip_address='$get_ip_add'";
    $result=mysqli_query($con,$cart_query);
    while($row=mysqli_fetch_array($result)){  //fetching products id
        $product_id=$row['product_id'];
        $select_products="SELECT * FROM products WHERE product_id='$product_id'"; 
        $result_products=mysqli_query($con,$select_products);
        while($row_product_price=mysqli_fetch_array($result_products)){
            $product_price=array($row_product_price['product_price']); //storing prices of each product in a cart in array
    ...
    while($row_query=mysqli_fetch_array($result_query)){
        ...
        if(!isset($_GET['edit_account'])){
            if(!isset($_GET['my_orders'])){
                if(!isset($_GET['delete_accountt'])){
                    $get_orders="SELECT * FROM `user_orders` WHERE user_id=$user_id AND order_status='pending'";
                    $result_orders_query=mysqli_query($con,$get_orders);
                    $row_count=mysqli_num_rows($result_orders_query);
                    if($row_count>0){
                        echo "<h3 class='text-center text-success mt-5 mb-2'>You have <span class='text-center'>$row_count</span> pending orders</h3><p class='text-center'><a href='profile.php?my_orders' class='text-dark'>Order Details</a></p>";
                    }else{
